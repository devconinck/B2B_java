    ...
    private void processOrderData() {
        try (CSVReader reader = new CSVReader(new FileReader(orderCSVFile))) {
            ...
            while ((line = reader.readNext()) != null && !line[0].equals(";;;;;;;;;")) {
                ...
                orders.add(new Order(orderId, syncId, customerId, orderReference, orderDateTime, lastPaymentReminder, netAmount, taxAmount, totalAmount, currency));
            }
    ...
    private void processOrderItemData() {
        try (CSVReader reader = new CSVReader(new FileReader(orderItemCSVFile))) {
            ...
            while ((line = reader.readNext()) != null && !line[0].equals(";;;;;;;;;")) {
                ...
                orderItems.add(new OrderItem(orderId, orderItemId, syncId, productId, quantity, unitOfMeasureId, netPrice, netAmount));
            }
            GenericDaoJpa.startTransaction();
    ...
    private void processProductPriceData() {
        try (CSVReader reader = new CSVReader(new FileReader(productPriceCSVFile))) {
            ...
            while ((line = reader.readNext()) != null && !line[0].equals(";;;;;;;;;")) {
                ...
                productPrices.add(new ProductPrice(productId, currencyId, syncId, price, unitOfMeasureId, syncDateTime, quantity));
            }
            GenericDaoJpa.startTransaction();
    ...
    private void processProductDescriptionData() {
        try (CSVReader reader = new CSVReader(new FileReader(productDescriptionCSVFile))) {
            ...
            while ((line = reader.readNext()) != null && !line[0].equals(";;;;;;;;;")) {
                ...
                productDescriptions.add(new ProductDescription(productId, languageId, syncId, productName, productListerDescription, productShortDescription, productLongDescription));
            }
            GenericDaoJpa.startTransaction();
    ...
    private void processProductUnitOfMeasureConversionData() {
        try (CSVReader reader = new CSVReader(new FileReader(productUnitOfMeasureConversionCSVFile))) {
            ...
            while ((line = reader.readNext()) != null && !line[0].equals(";;;;;;;;;")) {
                ...
                productUnitConversions.add(new ProductUnitOfMeasureConversion(productId, syncId, formUnitOfMeasure, toUnitOfMeasure, formQuantity, toQuantity));
            }
            GenericDaoJpa.startTransaction();
