...
public class OrdersFilterController extends FilterController<OrderDTO>{
	...
	private VBox createDatePicker() {
		...
		datePickerFrom.setEditable(true);
		...
		Button reset = new Button("Reset date");
		reset.setOnAction(event -> {
			this.fromDate = LocalDate.MIN.plusDays(1);
			this.untilDate = LocalDate.MAX.minusDays(1);
			runAllFilters();
			datePickerFrom.getEditor().clear();
			datePickerUntil.getEditor().clear();
		});
